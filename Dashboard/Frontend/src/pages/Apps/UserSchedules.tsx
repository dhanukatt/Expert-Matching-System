...
const UserSchedules = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get('http://localhost:8070/schedule/getUserSchedules', {
                    headers: {
    ...
    useEffect(() => {
        setFilteredItems(() => {
            return requests.filter((item: any) => {
                return item.expertId.firstName.toLowerCase().includes(search.toLowerCase());
            });
        });
    }, [search, requests]);
                                        ...
                                        <tr key={request._id}>
                                            <td>
                                                <div className="flex items-center w-max">
                                                    {request.expertId.profilePicture && (
                                                        <div className="w-max">
                                                            <img src={request.expertId.profilePicture} className="h-8 w-8 rounded-full object-cover ltr:mr-2 rtl:ml-2" alt="avatar" />
                                                        </div>
                                                    )}
                                                    {!request.expertId.profilePicture && request.name && (
